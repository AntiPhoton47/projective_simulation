...
# %% auto #0
__all__ = ['decay_toward_uniform', 'decay_toward_stationary', 'logit_bias', 'uniquify_percept_categories']

# %% ../../nbs/lib_nbs/methods/01_transforms.ipynb #0f769033-7e42-4b16-8763-08eadb1c12d3
import numpy as np
def _softmax(beta, x):
        """
...
# %% ../../nbs/lib_nbs/methods/01_transforms.ipynb #5385d425-0b77-4374-b963-83e2a9fae896
def _logistic(x, L:float = 1,k:float = 1,x_shift = 0,y_shift:float = 0):
    return L/(1 + np.exp(-k*(x-x_shift))) + y_shift

# %% ../../nbs/lib_nbs/methods/01_transforms.ipynb #e9bd6cb2-b86d-458e-98df-c578e5bba89d
def _exponentiated_shift(x, k, epsilon = 0.0001):
    return (x + epsilon) ** (np.exp(-k)) / ((1 + 2*epsilon) ** (np.exp(-k)))

# %% ../../nbs/lib_nbs/methods/01_transforms.ipynb #1d12519c-2ffc-4128-a092-6717f8545224
import numpy as np
...
# %% ../../nbs/lib_nbs/methods/01_transforms.ipynb #63c49ff7
import numpy as np
...
# %% ../../nbs/lib_nbs/methods/01_transforms.ipynb #13aedc1e
from numpy.typing import NDArray
def decay_toward_stationary(pmfs: NDArray[np.float64],                      #Array of shape (N, K) or (K,), where each row sums to 1.
                             gammas: NDArray[np.float64]|float,                    #Array of shape (N,), or scalar in shape (1,) or (), each gamma ∈ [0, 1].
                             stationary_distribution: NDArray[np.float64]    #Row vector of shape (K,) (sums to 1) used as the target mix.
...
# %% ../../nbs/lib_nbs/methods/01_transforms.ipynb #7a5c88e9
...
# %% ../../nbs/lib_nbs/methods/01_transforms.ipynb #61640dfb-8f27-40bc-a8e3-0085221c5540
...
# %% ../../nbs/lib_nbs/methods/01_transforms.ipynb #85b3afeb
def uniquify_percept_categories(percepts: np.ndarray, category_sizes: list) -> np.ndarray:
    ...
    """
    shifts = np.concatenate(([0], category_sizes[:-1]))
    unique_percepts = percepts + shifts
    return unique_percepts
