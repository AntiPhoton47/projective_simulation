...
# %% auto 0
__all__ = ['Factorized_POMDP', 'Factorized_Noisy_Cycle', 'Series_Recognition_Probe']
...
class Series_Recognition_Probe(Factorized_POMDP):
    """
    ...
    def __init__(self,
        ...
        needs_novel = (probe_position == -1)
        needed_total = needed_for_series + (1 if needs_novel else 0)

        if needed_total > total_items:
            raise ValueError(
                f"Not enough distinct items: need {needed_total}, but only {total_items} possible "
                f"({item_category_size}^{item_size})."
            )

        # Draw distinct integer IDs in [0, total_items) and decode them to items on-the-fly.
        sampled_idx = self.rng.choice(total_items, size=needed_total, replace=False)
        ...
        novel_item = sampled_items[-1, :] if needs_novel else None
        ...
        def _sample_binomial_gap(mu_: float, var_: float) -> int:
            """
            ...
            """
            if mu_ <= 0:
                return 1  # enforce at least one timeout even if mean is zero/tiny
        ...
        for s in range(num_series):
            ...
            if s < num_series - 1:
                gap_len = _sample_binomial_gap(mu, var)
        ...
        for _ in range(probe_cue_duration):
            percept_sequence.append(np.full(item_size, probe_signal_token, dtype=int))
        ...
        self.probe_item = probe.copy()                                  # type: ignore - None is only used if needs_novel, and is replaced with NDarray(,item_size) in this condition
